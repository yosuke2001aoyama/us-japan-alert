...
const principalCommunicationPattern =
  /\b(?:address(?:es|ed)?|announc(?:e|es|ed|ement)|brief(?:s|ed|ing)?|comment(?:s|ed)?|gaggles?|interview(?:s|ed)?|meet(?:s|ing)?|post(?:s|ed)?|press conference|press release|readout|remark(?:s|ed)?|respond(?:s|ed)?|say(?:s|ing)?|said|speech|speak(?:s|ing)?|statement|testif(?:y|ies|ied)|transcript|truth social|wrote|condolences?|pray(?:s|ed|ing|ers)?|commemorat(?:e|es|ed|ing)|remember(?:s|ed|ing)?|mourn(?:s|ed|ing)?|stands? with|solidarity)\b|会見|会談|発言|発表|表明|談話|声明|挨拶|演説|答弁|投稿|発信|報道発表|インタビュー|記者団|訓示|寄稿|お見舞い|哀悼|祈り|連帯/i;
const policyStatementPattern =
  /\b(?:statement|remarks?|gaggles?|press conference|press release|testif(?:y|ies|ied)|transcript|briefing)\b|会見|発言|談話|声明|答弁|記者団|報道発表|記者発表/i;
